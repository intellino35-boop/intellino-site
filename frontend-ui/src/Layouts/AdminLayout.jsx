...
export default function AdminLayout({ title, actions, children }) {
    ...
    const admin = auth.admin ?? { unread: 0, resources: [], blockTypes: [] };
    const { can } = auth;
    // Menu mobile ouvert « pour une page » : il se referme tout seul quand l'adresse change.
    const [openOn, setOpenOn] = useState(null);
    const open = openOn === path;
    const setOpen = (value) => setOpenOn(value ? path : null);
            ...
            <nav className="flex-1 space-y-1">
                ...
                {can('blocks') && (
                    <NavItem
                        href="/admin/blocs"
                        icon={LayoutList}
                        active={path === '/admin/blocs' || asArray(admin.blockTypes).some((t) => path.startsWith(`/admin/contenu/${t}`))}
                    >
                        Blocs des pages
                    </NavItem>
                )}
    ...
    return (
        <div className="min-h-screen bg-[#080808] lg:flex">
            ...
            <div className="min-w-0 flex-1">
                ...
                {/* Messages de succès / d'erreur : notifications Material UI (NotifyContext). */}
