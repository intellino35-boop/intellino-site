...
function RealisationView({ realisation: r, others }) {
    return (
        <SiteLayout>
            <Head title={r.title} />
            <PageHero
...
export default function Page() {
    ...
    const state = useApi(() => realisationsService.show(slug), [slug]);

    return (
        <DataPage state={state} title="Réalisation" isMissing={(data) => !data?.item}>
            {(data) => <RealisationView realisation={data.item} others={data.others} />}
        </DataPage>
    );
}
