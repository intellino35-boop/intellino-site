...
function PostView({ post, related }) {
    return (
        <SiteLayout>
...
/** Page : article chargé depuis l'API selon le slug (404 → page « introuvable »). */
export default function Page() {
    const { slug } = useParams();
    const state = useApi(() => postsService.show(slug), [slug]);

    return (
        <DataPage state={state} title="Article" isMissing={(data) => !data?.item}>
            {(data) => <PostView post={data.item} related={data.related} />}
        </DataPage>
    );
}
