...
import { useBlockTuples, useSite } from '../hooks/useSite';
...
function Solutions({ solutions, status }) {
    return (
        <section id="solutions" className="bg-[#080808] py-24">
            <div className="mx-auto max-w-7xl px-4">
                ...
                />
                <AsyncContent state={status} className="py-12">
...
function Products({ products, softwares, status }) {
    return (
...
function Realisations({ realisations, posts, status }) {
    return (
            ...
            <div className="mx-auto max-w-7xl px-4">
                ...
                <AsyncContent state={status} className="py-12">
                    <div className="mb-10 grid gap-6 md:grid-cols-2">
                        {asArray(realisations).map((r, i) => (
                            <Reveal key={r.id ?? r.slug} delay={i * 0.08}>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    {asArray(posts).map((post, i) => (
                        <Reveal key={post.id ?? post.slug} delay={i * 0.1}>
                            <PostCard post={post} />
                        </Reveal>
                    ))}
                </div>
...
export default function Home() {
    // Données de l'accueil : GET /api/home (solutions, produits, logiciels, réalisations, articles).
    const home = useApi(() => siteService.home(), []);
    const { solutions, products, softwares, realisations, posts } = home.data ?? {};
    ...
    return (
        <SiteLayout>
            ...
            <Hero />
            <About />
            <Solutions solutions={solutions} status={home} />
            <TechnologyLab />
            <Products products={products} softwares={softwares} status={home} />
            <WhyUs />
            <Realisations realisations={realisations} posts={posts} status={home} />
            <Contact />
        </SiteLayout>
    );
}
