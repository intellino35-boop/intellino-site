...
function SoftwareView({ software, others }) {
        ...
        <SiteLayout>
            ...
            <section className="bg-[#080808] pb-24">
                <div className="mx-auto grid max-w-7xl items-start gap-10 px-4 lg:grid-cols-5">
                    ...
                    {asArray(software.features).length > 0 && (
                        <Reveal x={30} y={0} className="lg:col-span-2">
                            <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-transparent p-7">
                                ...
                                <ul className="space-y-3">
                                    {asArray(software.features).map((f) => (
...
export default function Page() {
    ...
    const state = useApi(() => softwaresService.show(slug), [slug]);

    return (
        <DataPage state={state} title="Logiciel" isMissing={(data) => !data?.item}>
            {(data) => <SoftwareView software={data.item} others={data.others} />}
        </DataPage>
    );
}
